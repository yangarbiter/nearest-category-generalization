...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def get_constraints(np.ndarray[DTYPE_t, ndim=2] trnX, np.ndarray[DTYPE_t, ndim=1] tar_x):
    """
    n_closest: 
    """
    cdef int i, j, k
    cdef int n_points = trnX.shape[0], m = trnX.shape[1]
    ...
    for i in range(n_points):
        cx_get_half_space(trnX_view[i], tar_x, w_view, m)
        for k in range(m):
            G_view[i, k] = w_view[k]
        h_view[i] = w_view[m]
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def get_constraints_ori(np.ndarray[DTYPE_t, ndim=2] trnX, np.ndarray tuple_x,
